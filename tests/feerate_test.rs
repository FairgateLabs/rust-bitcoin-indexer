...
mod utils;
use crate::utils::{clear_output, get_indexer_store, wait_for_port_available};
...
fn test_get_estimated_fee_rate_with_seven_transactions() -> Result<(), anyhow::Error> {
    ...
    for i in 0..7 {
        ...
        let (funding_tx, vout) =
            bitcoin_client.fund_address(&address, Amount::from_sat(100_000))?;
    ...
    for (i, (prev_txid, prev_vout, private_key, prev_script_pubkey, public_key)) in
        utxo_data.iter().enumerate()
    {
        let output_secret = SecretKey::new(&mut OsRng);
        let output_secp_pubkey =
            bitcoin::secp256k1::PublicKey::from_secret_key(&secp, &output_secret);
        ...
        final_tx.input[0]
            .witness
            .push(final_sig_push_bytes.as_bytes());
        ...
        println!(
            "  Transaction {}: fee={} sat, vsize={}, rate={} sat/vB (target: {})",
            i, exact_fee, final_vsize, final_fee_rate, target_fee_rate
        );

        // Verify exact fee rate
        assert_eq!(
            final_fee_rate, target_fee_rate,
            "Transaction {} should have exact fee rate {}",
            i, target_fee_rate
        );
    ...
    let indexer = Indexer::new(bitcoin_client, store, Some(IndexerSettings::new(Some(100))))?;
    ...
    assert_eq!(
        block.txs.len(),
        8,
        "Block should have 8 transactions (1 coinbase + 7 spending)"
    );
    assert!(
    ...
    assert_eq!(
        estimated_fee_rate, 40,
        "get_estimated_fee_rate should return exactly 40 sat/vB"
    );
    ...
    clear_output();
    assert!(
        wait_for_port_available(5),
        "Port 18443 should be available after container stop"
    );
    Ok(())
}
...
fn test_get_estimated_fee_rate_indexer_not_synced() -> Result<(), anyhow::Error> {
    ...
    let new_indexer = Indexer {
        bitcoin_client: bitcoin_client_2,
