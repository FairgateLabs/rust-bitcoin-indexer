...
#[test]
fn test_get_estimated_fee_rate_block_not_found() -> Result<(), anyhow::Error> {
    use bitcoin_indexer::errors::IndexerError;

    let mut bitcoin_client = MockBitcoinClient::new();
    let store = get_indexer_store();

    // Mock the bitcoin client to return blockchain height of 1000
    // This ensures the indexer and blockchain heights match (avoiding IndexerNotSynced error)
    bitcoin_client
        .expect_get_best_block()
        .returning(|| Ok(1000));

    // Create an indexer but don't process any blocks
    // This means the store will be empty (no best block saved)
    let indexer = Indexer {
        bitcoin_client,
        store,
    };

    // Manually set the last synced height to match the blockchain height
    // to avoid the IndexerNotSynced error
    indexer.store.save_last_synced_height(1000)?;

    // Try to get estimated fee rate when no blocks exist in the store
    let result = indexer.get_estimated_fee_rate();

    // Should return IndexerError::BlockNotFound
    // because store.get_best_block() returns None
    assert!(result.is_err());
    match result.unwrap_err() {
