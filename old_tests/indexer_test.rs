...
fn test_configuration_missing_optional_fields() -> Result<(), anyhow::Error> {
    ...
    assert!(
        config_with_none_settings.settings.is_none(),
        "settings should be None when not specified"
    );
    assert!(
        config_with_none_settings.log_level.is_none(),
        "log_level should be None when not specified"
    );
    ...
    assert_eq!(
        ...
        Some(0),
        "default checkpoint height should be 0"
    );
...
fn test_indexersettings_defaults() {
    ...
    assert!(
        default_settings.checkpoint_height.is_some(),
        "checkpoint_height should have a value"
    );
    assert_eq!(
    ...
    assert!(
        settings_with_none.checkpoint_height.is_none(),
        "IndexerSettings::new(None) should have None checkpoint"
    );

    // But Default::default() should have Some(0)
    assert_ne!(
        settings_with_none.checkpoint_height, default_settings.checkpoint_height,
...
fn test_indexed_height_exceeds_blockchain_height() -> Result<(), anyhow::Error> {
    ...
    assert!(result.is_err());
    assert!(matches!(result, Err(IndexerError::InconsistentBlockchain)));
...
fn test_database_corrupted_missing_block_hash_for_height() -> Result<(), anyhow::Error> {
    ...
    assert!(result.is_err());
    assert!(matches!(result, Err(IndexerError::DatabaseCorrupted)));
